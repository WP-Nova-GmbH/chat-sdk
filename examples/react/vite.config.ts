...
function novaTokenProxyPlugin(env: ExampleEnv): Plugin {
    return {
        ...
        configResolved(config) {
            if (!env.testEmail) {
                config.logger.error(
                    "[nova example] NOVA_TEST_EMAIL is missing. Set it in examples/react/.env " +
                        "(see examples/react/.env.example); it supplies the simulated " +
...
function originForRequest(request: IncomingMessage): string {
    const host = request.headers.host || "127.0.0.1:4311";
    return `http://${host}`;
}
